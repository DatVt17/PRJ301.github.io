...
public static User getAllUserbyID() {
     User u = new User();
        try {
            Connection conn = DAO.DB.getConnection();
            PreparedStatement st = conn.prepareStatement("select * from tblUsers where userID=?");
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public static void addNewUser(User u) {
        try {
            ...
            PreparedStatement st = conn.prepareStatement("insert into tblUsers values (?,?,?,?,?,?,?,?)");
